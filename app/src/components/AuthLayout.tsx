...
function useMenuItems(userRole?: string) {
  return useMemo(() => {
    const items = [
      { icon: LayoutDashboard, label: "Dashboard", path: "/" },
      { icon: Route, label: "Routes", path: "/routes" },
      { icon: Bus, label: "Trips", path: "/trips" },
      { icon: ClipboardCheck, label: "Reports", path: "/report" },
      { icon: QrCode, label: "Scan QR", path: "/scan" },
    ];

    if (userRole === "driver") {
      items.push({ icon: Bus, label: "Driver Mode", path: "/driver" });
    }
    ...
    items.push({ icon: UserCircle, label: "Profile", path: "/profile" });
        ...
        <div className="flex flex-col items-center gap-8 p-8 max-w-md w-full">
          <div className="flex flex-col items-center gap-6">
            ...
            <p className="text-sm text-muted-foreground text-center max-w-sm">
              Access to Sakay Naga requires authentication. Sign in to report
              capacity, view routes, and more.
            </p>
          </div>
          <Button
            onClick={() => {
            ...
            <div className="flex items-center gap-3 px-2 transition-all w-full">
              ...
              {!isCollapsed ? (
                <div className="flex items-center gap-2 min-w-0">
                  <span className="font-semibold tracking-tight truncate text-emerald-600">
                    Sakay Naga
                  </span>
                </div>
              ) : null}
